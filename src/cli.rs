...
fn update_installer_urls(normalized_tag: &str, script: &str) -> (String, String) {
    let repository = "Pimpmuckl/destructive_command_guard";
    (
        format!("https://raw.githubusercontent.com/{repository}/{normalized_tag}/{script}"),
        format!(
            "https://github.com/{repository}/releases/download/{normalized_tag}/{script}.sha256"
        ),
    )
}
...
fn self_update_unix(update: UpdateCommand) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let (script_url, sha_url) = update_installer_urls(&normalized_tag, "install.sh");
...
fn self_update_windows(update: UpdateCommand) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let requested_version = update.version.clone();
    let normalized_tag = update_installer_tag(requested_version.as_deref())?;
    let (script_url, sha_url) = update_installer_urls(&normalized_tag, "install.ps1");
...
mod tests {
    ...
    #[test]
    fn test_update_installer_urls_use_fork_release() {
        assert_eq!(
            update_installer_urls("v0.6.8-codexpp.1", "install.sh"),
            (
                "https://raw.githubusercontent.com/Pimpmuckl/destructive_command_guard/v0.6.8-codexpp.1/install.sh".to_string(),
                "https://github.com/Pimpmuckl/destructive_command_guard/releases/download/v0.6.8-codexpp.1/install.sh.sha256".to_string(),
            )
        );
    }
